...
import {
  Download, Copy, Shield, ExternalLink, ChevronDown, ChevronRight, Search,
} from "lucide-react";
...
function FindingDetail({ finding }: { finding: Finding }) {
  return (
    <div className="p-4 bg-muted/30 rounded-md space-y-4 animate-fade-in">
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-3 mt-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p className="text-foreground break-all">{finding.url}</p>
            </div>
            <div>
              <span className="text-xs text-muted-foreground">Method</span>
              <p className="text-foreground">{finding.method}</p>
            </div>
            <div>
              <span className="text-xs text-muted-foreground">Parameter</span>
              <p className="text-foreground font-mono text-xs">{finding.parameter || "—"}</p>
            </div>
          </div>
        </TabsContent>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <Button
                  ...
                  onClick={() => { navigator.clipboard.writeText(finding.payload); toast.success("Copied"); }}
                >
                  <Copy className="h-3 w-3 mr-1" /> Copy
                </Button>
              </div>
              <pre className="p-3 rounded-md bg-background border border-border text-xs font-mono overflow-x-auto text-foreground">
                {finding.payload}
              </pre>
            </div>
          )}
          {finding.evidence && (
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <Button
                  variant="ghost" size="sm" className="h-6 text-[10px]"
                  onClick={() => { navigator.clipboard.writeText(finding.evidence); toast.success("Copied"); }}
                >
                  <Copy className="h-3 w-3 mr-1" /> Copy
                </Button>
              </div>
        ...
        <TabsContent value="ai" className="space-y-3 mt-3">
          ...
          {finding.businessImpact && (
            <div>
              <span className="text-xs text-muted-foreground font-medium">Business Impact</span>
              <p className="text-sm text-muted-foreground mt-1">{finding.businessImpact}</p>
            </div>
          )}
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs text-muted-foreground">
                {Math.round((finding.falsePositiveConfidence ?? 0) * 100)}% chance false positive
              </span>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="remediation" className="space-y-3 mt-3">
          <p className="text-sm text-muted-foreground whitespace-pre-line">{finding.aiRemediation || "Remediation guidance pending."}</p>
...
const Report = () => {
  ...
  const filteredFindings = useMemo(() => {
    ...
    return f;
  }, [findings, severityFilter, typeFilter, confirmedOnly, searchQuery, sortBy]);
  ...
  const exportPDF = async () => {
    try {
      ...
      pdf.save(`origin-report-${scanId}.pdf`);
      ...
      <div id="report-content" className="space-y-6 max-w-6xl">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">Security Assessment Report</h1>
              <p className="text-xs text-muted-foreground">
                {scan?.targetUrl} · {scan?.scanProfile} · {scan?.createdAt?.toDate?.().toLocaleDateString()}
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button onClick={exportPDF} className="gap-2" size="sm">
              <Download className="h-4 w-4" /> Download PDF
            </Button>
            <Button variant="outline" onClick={copyLink} size="sm" className="gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {([
            { sev: "critical", count: stats.critical },
            { sev: "high", count: stats.high },
            { sev: "medium", count: stats.medium },
            { sev: "low", count: stats.low },
            { sev: "info", count: stats.info },
          ] as const).map((s) => (
            <Card key={s.sev} className="bg-card border-border">
          ...
          <Card className="md:col-span-2 bg-card border-border">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground leading-relaxed">
                {scan?.aiSummary || `Security scan of ${scan?.targetUrl} completed with ${stats.total} findings across ${nodes.length} discovered endpoints. Review the findings below for detailed vulnerability analysis and remediation guidance.`}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            { label: "Scan Duration", value: scan?.completedAt && scan?.createdAt ? `${Math.round((scan.completedAt.toDate().getTime() - scan.createdAt.toDate().getTime()) / 60000)} min` : "—" },
        ...
        <Card className="bg-card border-border">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Findings ({filteredFindings.length})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="confirmed" className="text-xs text-muted-foreground">Confirmed Only</Label>
              </div>
            ...
            {filteredFindings.length === 0 ? (
              ...
              <div className="space-y-1">
                {/* Desktop header */}
                <div className="hidden md:grid grid-cols-[80px_100px_1fr_160px_60px_60px_80px] gap-2 text-[10px] text-muted-foreground font-medium py-1 px-2">
                  <span>Severity</span><span>Type</span><span>Title</span><span>URL</span><span>CVSS</span><span>Risk</span><span></span>
                </div>
                {filteredFindings.map((f) => (
                  <div key={f.findingId}>
                    <button
                      onClick={() => setExpandedId(expandedId === f.findingId ? null : f.findingId)}
                      className={`w-full text-left hidden md:grid grid-cols-[80px_100px_1fr_160px_60px_60px_80px] gap-2 items-center py-2 px-2 rounded-md hover:bg-muted/30 transition-colors ${
                        f.severity === "critical" ? "border-l-2 border-l-severity-critical" : ""
                      }`}
                    >
                      <SeverityBadge severity={f.severity} />
                      <Badge variant="outline" className="text-[10px] justify-center">{f.type}</Badge>
                      <span className="text-sm truncate text-foreground">{f.title}</span>
                      <span className="text-xs truncate text-muted-foreground">{f.url}</span>
                      <span className="text-xs font-mono text-muted-foreground">{f.cvssScore?.toFixed(1)}</span>
                      <span className="text-xs font-mono text-muted-foreground">{f.businessRiskScore ?? "—"}</span>
                      <span className="text-xs text-primary flex items-center gap-1">
                        Details {expandedId === f.findingId ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                      </span>
                    </button>
                    {/* Mobile card */}
                    <button
                      onClick={() => setExpandedId(expandedId === f.findingId ? null : f.findingId)}
                      className={`w-full text-left md:hidden p-3 rounded-md bg-muted/20 space-y-2 ${
                        f.severity === "critical" ? "border-l-2 border-l-severity-critical" : ""
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <SeverityBadge severity={f.severity} />
                        <span className="text-sm font-medium truncate text-foreground">{f.title}</span>
                      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      {scan.roles.map((r) => (
                        <th key={r.name} className="text-center py-2 px-3 text-xs text-muted-foreground font-medium">{r.name}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {nodes.filter((n) => n.type !== "asset").slice(0, 20).map((n) => (
                        ...
                        {scan.roles.map((r) => (
                          <td key={r.name} className="py-2 px-3 text-center">
                            <Badge variant="outline" className={`text-[10px] ${
                              n.vulnerable
                                ? "text-severity-high border-severity-high/30 bg-severity-high/10"
            ...
            <CardContent>
              <div className="space-y-1">
                {apiNodes.map((n) => {
                  ...
                  return (
                    <div key={n.url} className="flex items-center gap-3 p-2 rounded-md hover:bg-muted/30">
                      ...
                      <Badge variant={n.vulnerable ? "destructive" : "secondary"} className="text-[10px]">
                        {n.vulnerable ? "Vulnerable" : n.scanned ? "Clean" : "Untested"}
                      </Badge>
                      {findingsForUrl.length > 0 && (
                        <span className="text-xs text-muted-foreground">{findingsForUrl.length} findings</span>
                      )}
                    </div>
                  );
                })}
                      ...
                      <tr key={cat.id} className="border-b border-border/50">
                        ...
                        <td className="py-2 px-3 text-center">
                          <Badge variant="outline" className={`text-[10px] ${
                            ...
                            {status === "vulnerable" ? "Vulnerable" : status === "clean" ? "Clean" : "Not Tested"}
                          </Badge>
                        </td>
