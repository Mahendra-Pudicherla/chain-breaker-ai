...
const ScanHistory = () => {
  ...
  useEffect(() => {
    if (!user?.uid) return;
    const unsub = onUserScans(user.uid, (s) => {
      setScans(s);
      setLoading(false);
    });
    return unsub;
  ...
  const filtered = useMemo(() => {
    let f = [...scans];
    if (search) f = f.filter((s) => s.targetUrl.toLowerCase().includes(search.toLowerCase()));
    ...
    return f;
  }, [scans, search, statusFilter, profileFilter, sortOrder]);

  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await deleteScanFull(deleteId);
      ...
      <div className="space-y-6 max-w-6xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent>
              ...
              {Object.values(ScanStatus).map((s) => (
                <SelectItem key={s} value={s} className="capitalize">{s}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Select value={profileFilter} onValueChange={setProfileFilter}>
            <SelectTrigger className="w-32"><SelectValue placeholder="Profile" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Profiles</SelectItem>
              {Object.values(ScanProfile).map((p) => (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 space-y-3">
            ...
            <Button onClick={() => navigate("/scan/new")} className="gap-2">
              <Plus className="h-4 w-4" /> Start Scan
            </Button>
          </div>
        ) : (
          <>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the scan and all associated findings. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDelete} className="bg-severity-critical text-primary-foreground">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
