...
const Dashboard = () => {
  ...
  const lastScanDate = asDate((lastScan as any)?.createdAt);
  const lastScanTime = lastScanDate
  ...
  const statCards = [
    { title: "Total Scans", value: scans.length, icon: BarChart3, color: "bg-primary/10 text-primary" },
                        ...
                        {recentScans.map((scan) => (
                          <tr key={scan.scanId} className="border-b border-border/50 hover:bg-surface-elevated/50 transition-colors">
                            ...
                            <td className="py-2.5 text-right text-xs text-muted-foreground">
                              {asDate((scan as any).createdAt)
                                ? formatDistanceToNow(asDate((scan as any).createdAt) as Date, { addSuffix: true })
                                : "—"}
                            </td>
